...
- (void)viewDidLoad {
	...
	_pagingScrollView.backgroundColor = XKBGDefaultColor;
...
- (void)updateNavigation {
    ...
    NSUInteger numberOfPhotos = [self numberOfPhotos];
    if (!_right) {
        self.right = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 50, 44)];
        _right.backgroundColor = [UIColor clearColor];
        
        UILabel * total = [[UILabel alloc] initWithFrame:CGRectMake(25, 1
                                                                    , 25, 44)];
        total.backgroundColor = [UIColor clearColor];
        total.textColor =   [UIColor whiteColor]; //[UIColor colorFromHexString:@"#808080"];
        total.font = XKDefaultFontWithSize(13);
        total.text = [NSString stringWithFormat:@"/%lu",(unsigned long)numberOfPhotos];
        [_right addSubview:total];
        
        UILabel * select = [[UILabel alloc] initWithFrame:CGRectMake(0, 1, 25, 44)];
        select.tag = 336655;
        select.backgroundColor = [UIColor clearColor];
        select.font = XKDefaultFontWithSize(13);
        select.textColor =   [UIColor whiteColor];//XKMainToneColor_00b4b4;
        select.text = @"1";
        select.textAlignment = NSTextAlignmentRight;
        [_right addSubview:select];
        UIBarButtonItem * bar= [[UIBarButtonItem alloc] initWithCustomView:_right];
    ...
    if (_gridController) {
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
            UILabel * temp = (UILabel *)[self.right viewWithTag:336655];
            temp.text = [NSString stringWithFormat:@"%lu", (unsigned long)(_currentPageIndex+1)];
