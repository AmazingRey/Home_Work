    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                XKLog(@"%s:%d", __FUNCTION__, __LINE__);
                XKLog(@"Database busy, unable to close");
                return NO;
            }
            ...
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, 0x00)) !=0) {
                    XKLog(@"Closing leaked statement");
                    sqlite3_finalize(pStmt);
                }
            }
        }
        else if (SQLITE_OK != rc) {
            XKLog(@"error closing!: %d", rc);
        }
    }
    while (retry);
...
- (BOOL)rekeyWithData:(NSData *)keyData {
    ...
    if (rc != SQLITE_OK) {
        XKLog(@"error on rekey: %d", rc);
        XKLog(@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
    XKLog(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        XKLog(@"The FMDatabase %@ is not open.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        do {
            ...
            if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
                ...
                if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                    XKLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [self databasePath]);
                    XKLog(@"Database busy");
                    sqlite3_finalize(pStmt);
                    _isExecutingStatement = NO;
                    return nil;
                ...
                if (_logsErrors) {
                    XKLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                    XKLog(@"DB Query: %@", sql);
                    XKLog(@"DB Path: %@", _databasePath);
#ifndef NS_BLOCK_ASSERTIONS
                    if (_crashOnErrors) {
                        abort();
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (namedIdx > 0) {
                ...
                XKLog(@"Could not find index for %@", dictionaryKey);
            }
        }
    }
    else {
        ...
        while (idx < queryCount) {
            ...
            if (_traceExecution) {
                if ([obj isKindOfClass:[NSData class]]) {
                    XKLog(@"data: %ld bytes", (unsigned long)[(NSData*)obj length]);
                }
                else {
                    XKLog(@"obj: %@", obj);
                }
    ...
    if (idx != queryCount) {
        XKLog(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (idx != queryCount) {
        XKLog(@"Error: the bind count (%d) is not correct for the # of variables in the query (%d) (%@) (executeUpdate)", idx, queryCount, sql);
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return NO;
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (SQLITE_LOCKED == rc) {
                ...
                if (rc != SQLITE_LOCKED) {
                    XKLog(@"Unexpected result from sqlite3_reset (%d) eu", rc);
                }
            }
            usleep(FMDatabaseSQLiteBusyMicrosecondsTimeout);
            ...
            if (_logsErrors) {
                XKLog(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
                XKLog(@"DB Query: %@", sql);
            }
        }
            ...
            // uh oh.
            if (_logsErrors) {
                XKLog(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
                XKLog(@"DB Query: %@", sql);
            }
        }
        else {
            // wtf?
            if (_logsErrors) {
                XKLog(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
                XKLog(@"DB Query: %@", sql);
            }
        }
    ...
    if (closeErrorCode != SQLITE_OK) {
        if (_logsErrors) {
            XKLog(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
            XKLog(@"DB Query: %@", sql);
        }
    }
