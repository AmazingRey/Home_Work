...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        ...
        if (![_db open]) {
#endif
            XKLog(@"Could not create database queue for path %@", aPath);
            FMDBRelease(self);
            return 0x00;
        }
...
- (FMDatabase*)database {
    if (!_db) {
        ...
        if (![_db openWithFlags:_openFlags]) {
			...
			if (![db open])
#endif
            XKLog(@"FMDatabaseQueue could not reopen database for path %@", _path);
            FMDBRelease(_db);
            _db  = 0x00;
            return 0x00;
        }
...
- (void)inDatabase:(void (^)(FMDatabase *db))block {
    ...
    dispatch_sync(_queue, ^() {
        ...
        if ([db hasOpenResultSets]) {
            XKLog(@"Warning: there is at least one open result set around after performing [FMDatabaseQueue inDatabase:]");
        }
    });
