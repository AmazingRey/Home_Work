...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        XKLog(@"Warning: There seem to be no columns in this set.");
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (SQLITE_LOCKED == rc) {
                ...
                if (rc != SQLITE_LOCKED) {
                    XKLog(@"Unexpected result from sqlite3_reset (%d) rs", rc);
                }
            }
            ...
            if ([_parentDB busyRetryTimeout] && (numberOfRetries++ > [_parentDB busyRetryTimeout])) {
                ...
                XKLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
                XKLog(@"Database busy");
                break;
            }
        }
            ...
            XKLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
            break;
        } 
            ...
            // wtf?
            XKLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
            break;
        }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    XKLog(@"Warning: I could not find the column named '%@'.", columnName);
