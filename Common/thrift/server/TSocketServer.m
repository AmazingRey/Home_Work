   ...
   processorFactory: (id <TProcessorFactory>) processorFactory;
{
  ...
  if (socket) {
    ...
    if (CFSocketSetAddress(socket, (bridge_stub CFDataRef)address) != kCFSocketSuccess) {
      ...
      CFRelease(socket);
      XKLog(@"*** Could not bind to address");
      return nil;
    }
  } else {
    XKLog(@"*** No server socket");
    return nil;
  }
  ...
  XKLog(@"Listening on TCP port %d", port);
...
- (void) handleClientConnection: (NSFileHandle *) clientSocket
{
    ...
    @autoreleasepool {
        ...
        @catch (TTransportException * te) {
            //XKLog(@"Caught transport exception, abandoning client connection: %@", te);
        }
