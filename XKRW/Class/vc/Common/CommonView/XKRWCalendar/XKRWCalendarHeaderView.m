...
- (instancetype)initWithType:(XKRWCalendarHeaderType)type onClickPreMonthBlock:(void (^)(void))preMonth onClickNextMonthBlock:(void (^)(void))nextMonth onClickBackToTodayBlock:(void (^)(void))backToToday
{
    _preMonth = preMonth;
    ...
    if (self = [super init]) {
        ...
        if (type == XKRWCalendarHeaderTypeDefault) {
            
            self.frame = CGRectMake(0, 0, XKAppWidth, 40.f + LABEL_HEIGHT);
            self.backgroundColor = [UIColor whiteColor];
            
            _dateView = [[UIView alloc] initWithFrame:CGRectMake(0.f, 0.f, SCREEN_SIZE.width, 40.f)];
            ...
            [dateLabel setFont:XKDefaultNumEnFontWithSize(16)];
            ...
            weekdayView = [[UIView alloc] initWithFrame:CGRectMake(0.f, 40.f, XKAppWidth, LABEL_HEIGHT)];
            weekdayView.backgroundColor = XKMainSchemeColor;
            ...
            if (!self.date) {
                ...
                dateLabel.text = [self.date formatInXKRWCalendar];
            }
            
            _backToTodayButton = [[UIButton alloc] initWithFrame:CGRectMake(XKAppWidth - 100.f, 5.f, 100.f, 30.f)];
            [_backToTodayButton.titleLabel setFont:[UIFont systemFontOfSize:16]];
            [_backToTodayButton setTitleColor:XKMainSchemeColor forState:UIControlStateNormal];
        ...
        } else if (type == XKRWCalendarHeaderTypeSimple) {
...
{
    _date = date;
    dateLabel.text = [date formatInXKRWCalendar];
}
