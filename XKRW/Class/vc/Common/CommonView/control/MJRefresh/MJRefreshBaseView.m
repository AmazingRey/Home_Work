...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        // 4.箭头图片
        UIImageView *arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"monkeyUp"]];
        arrowImage.frame = CGRectMake(40, 5, 73, 35);
        arrowImage.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
        [self addSubview:_arrowImage = arrowImage];
        
        YLImageView *loadingImageView = [[YLImageView alloc]init];
        loadingImageView.image = [YLGIFImage imageNamed:@"monkey.gif"];
        XKLog(@"%@",loadingImageView.image);
        loadingImageView.bounds = arrowImage.bounds;
...
- (void)setFrame:(CGRect)frame
{
   ...
   // _activityView.center = _arrowImage.center;
    ...
    _loadingImageView.center = _arrowImage.center;
...
- (void)setState:(MJRefreshState)state
{
    ...
    switch (state) {
		case MJRefreshStateNormal: // 普通状态
            ...
            MJLog(@"%f,%f",_loadingImageView.frame.origin.x,_loadingImageView.frame.size.width);
		...
		case MJRefreshStateRefreshing:
            ...
            XKLog(@"%@",_loadingImageView.image);
              XKLog(@"%f,%f",_loadingImageView.frame.origin.x,_loadingImageView.frame.size.width);
//            _loadingImageView.hidden = NO;
            // 隐藏箭头
			_arrowImage.hidden = YES;
  
            _arrowImage.image = [UIImage imageNamed:@"monkeyUp"];
            // 通知代理
