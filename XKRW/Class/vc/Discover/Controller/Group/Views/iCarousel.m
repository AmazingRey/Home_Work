...
- (UIView *)containView:(UIView *)view
{
    ...
    UITapGestureRecognizer* Doubleclick = [[UITapGestureRecognizer alloc] initWithTarget:self 
                                                                                  action:@selector(DoubleClickResponder:)];
    Doubleclick.numberOfTapsRequired = 2;
    [containerView addGestureRecognizer:Doubleclick];
    [Doubleclick release];
    [tapGesture requireGestureRecognizerToFail:Doubleclick];//手势要点,责任链模式
...
- (void)step
{
    ...
    if (_toggle != 0.0f)
    {
        CGFloat toggleDuration = fminf(1.0f, fmaxf(0.0f, 1.0f / fabs(_startVelocity)));
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    if (newSuperview) 
    {
        if (self.currentItemIndex == 0 && [_delegate respondsToSelector:@selector(carouselCurrentItemIndexDidChange:)]) //第一次进入时
        {
...
#ifdef ICAROUSEL_IOS
#pragma mark
#pragma mark autoPlayPrecess

-(void)stepBystepScroll
{
    if (isBreakByUser) return;
    
    NSUInteger nextIndex = [self clampedIndex:self.currentItemIndex+1];
    [self scrollToItemAtIndex:nextIndex animated:YES];
}

-(void)setAutoPlay:(BOOL)is_auto_play
{
    if (self.numberOfItems <= 1) return; 
    if (autoPlay == is_auto_play) return;
    
    autoPlay = is_auto_play;
    
    autoPlay? [self timeOn] : [self timeOff];
}

//用户介入时，中断自动播放逻辑
-(void)autoplayBreakByUser
{
    isBreakByUser = YES;
    
    [NSObject cancelPreviousPerformRequestsWithTarget:self
                                             selector:@selector(resumeUserBreak)
                                               object:nil];
    
    [self performSelector:@selector(resumeUserBreak) withObject:nil afterDelay:2];
}

-(void)resumeUserBreak
{
    isBreakByUser = NO;
}
...
-(void)timeOn
{
    ...
    autoPlayTimer = [NSTimer scheduledTimerWithTimeInterval:period/1000
                                                     target:self
                                                   selector:@selector(stepBystepScroll)
                                                   userInfo:nil
                                                    repeats:YES];
    [autoPlayTimer retain];
}
...
- (BOOL)viewOrSuperview:(UIView *)view isKindOfClass:(Class)clazz
{
    if (view == nil || view == _contentView)
    {
        return NO;
    }
    else if ([view isKindOfClass:clazz])
    {
        return YES;
    }
    return [self viewOrSuperview:view.superview isKindOfClass:clazz];
}
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gesture
{
    ...
    {
        if (scrollDelegate) {
            return [scrollDelegate iCarousel:self scrollShouldBegin:(id)gesture];
        }
...
-(void)DoubleClickResponder:(UITapGestureRecognizer*)tapGesture
{
    //有用户介入时,取消上次的自动,延迟
    [self autoplayBreakByUser];
    
    if ([_delegate respondsToSelector:@selector(carousel:didDoubleClickAtIndex:)]) 
    {
        NSInteger index = [self indexOfItemView:[tapGesture.view.subviews lastObject]];
        [_delegate carousel:self didDoubleClickAtIndex:index];
    }
}
