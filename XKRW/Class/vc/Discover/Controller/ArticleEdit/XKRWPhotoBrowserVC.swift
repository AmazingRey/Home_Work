...
@objc protocol XKRWPhotoBrowserDelegate {
    optional func photoBrowserVC(vc: XKRWPhotoBrowserVC, deleteImageAtIndex index: Int) -> Void
}

class XKRWPhotoBrowserVC: XKRWBaseVC {
    
    weak var delegate: protocol<XKRWPhotoBrowserDelegate>?
...
//            view.addGestureRecognizer(gesture)
        ...
        self.topBar = {
            let bar = UIView(frame: CGRectMake(0, 0, UI_SCREEN_WIDTH, 64))
            ...
            let deleteButton = UIButton(frame: CGRectMake(UI_SCREEN_WIDTH - 60, 20, 60, 44))
    ...
    func loadContent() {
        ...
        if self.images != nil {
            ...
            for image in self.images! {
                ...
                view.tag = tag
                //self.scrollView.width * (max(image.size.height , image.size.width)/min(image.size.height,image.size.width))
                view.frame = CGRectMake(0, 0, self.scrollView.width, self.scrollView.height)
...
extension XKRWPhotoBrowserVC: UIScrollViewDelegate {
...
extension XKRWPhotoBrowserVC: UIActionSheetDelegate {
